...
export const DATA_MODE: 'rest' | 'hybrid' = 'hybrid'; // Alpaca has much better WebSocket support

// Mock data mode for testing (set to false for production)
...
interface UpdateIntervals {
  WEBSOCKET_RECONNECT_MS: number;
  HISTORICAL_REFRESH_MS: number;
  PRICE_UPDATE_DEBOUNCE_MS: number;
  FIRST_5M_FETCH_TIME: string;
  REST_QUOTE_POLL_MS: number;
}
...
export const UPDATE_INTERVALS: UpdateIntervals = {
  ...
  REST_QUOTE_POLL_MS: 15000, // 15s in hybrid
};
...
interface MarketConfig {
  ...
  MARKET_OPEN_MINUTE: number;
  MARKET_CLOSE_HOUR: number;
  MARKET_CLOSE_MINUTE: number;
  ORB_START_HOUR: number;
  ORB_START_MINUTE: number;
}
