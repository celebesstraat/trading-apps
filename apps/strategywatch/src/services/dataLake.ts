...
import { Tick, Bar, StrategyScore, MovingAverages, RVolResult, VRSResult, FinnhubCandles, PriceData, HistoricalCandlesByDate, APIStatus, ConnectionState } from '../types/types';
 ...
 */
export const initDataLake = async (): Promise<IDBDatabase> => {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event: IDBVersionChangeEvent) => {
      ...
      (event.target as IDBOpenDBRequest).transaction.onerror = (error) => {
      ...
      const createStore = (name: string, keyPath: string | string[], indexes: (string | [string, string | string[], IDBIndexParameters])[] = []) => {
        try {
          ...
          if (!db.objectStoreNames.contains(name)) {
            ...
            // Store exists, get it from the transaction to add missing indexes
            const transaction = (event.target as IDBOpenDBRequest).transaction;
            store = transaction.objectStore(name);
            console.log(`[DataLake] Store ${name} already exists, checking for missing indexes`);
          }

          // Create/verify indexes (works for both new and existing stores)
          indexes.forEach(index => {
            try {
              const indexName = Array.isArray(index) ? index[0] : index;
              ...
              if (!store.indexNames.contains(indexName)) {
                ...
                console.log(`[DataLake] Index ${indexName} already exists on store ${name}`);
              }
            } catch (indexError) {
      ...
      createStore(STORES.TICKS, ['ticker', 'timestamp'], [
        ...
        ['timestamp_ticker', ['timestamp', 'ticker']]
      ]);
      ...
      createStore(STORES.INDICATORS, ['ticker', 'type', 'date'], [
        ...
        ['ticker_type_date', ['ticker', 'type', 'date']],
        'lastUpdated'
      ]);
      ...
      createStore(STORES.STRATEGY_RESULTS, ['ticker', 'strategy', 'timestamp'], [
        'ticker',
        'strategy',
        ['ticker_strategy', ['ticker', 'strategy']],
        ['ticker_strategy_timestamp', ['ticker', 'strategy', 'timestamp']],
        'timestamp'
      ]);
  ...
  return async () => {
    if (dbPromise) {
      try {
        ...
        dbPromise = null; // Reset failed promise
      }
    }
    ...
    try {
      dbPromise = initDataLake().catch(error => {
        ...
        // If version change error, suggest reset
        if (error.name === 'VersionError' || error.message.includes('Version change')) {
          console.warn('[DataLake] Version conflict detected. Try clearing browser data or run: window.dataLakeUtils.forceSchemaUpgrade()');
        }
 ...
 */
export const storeTicks = async (ticker: string, ticks: Tick[]): Promise<void> => {
  let db = null;
  try {
 ...
 */
export const getRecentTicks = async (ticker: string, limit = 1000, since: number | null = null): Promise<Tick[]> => {
  try {
 ...
 */
export const buildMinuteCandle = async (ticker: string, timestamp: number): Promise<Bar | null> => {
  try {
 ...
 * @returns {Promise<void>}
 */
export const storeMinuteCandle = async (ticker: string, candle: Bar): Promise<void> => {
  try {
    const db = await getDatabase();
 ...
 * @returns {Promise<Array>}
 */
export const getMinuteCandles = async (ticker: string, startTime: number, endTime: number): Promise<Bar[]> => {
  try {
    const db = await getDatabase();
    const transaction = db.transaction([STORES.MINUTE_CANDLES], 'readonly');
    const store = transaction.objectStore(STORES.MINUTE_CANDLES);
    const index = store.index('ticker_timestamp');
 ...
 * @returns {Promise<void>}
 */
export const storeFiveMinuteCandles = async (ticker: string, date: string, candles: Bar[]): Promise<void> => {
  try {
    const db = await getDatabase();
 ...
 * @returns {Promise<void>}
 */
export const storeDailyCandles = async (ticker: string, candles: Bar[]): Promise<void> => {
  try {
    const db = await getDatabase();
 ...
 * @returns {Promise<void>}
 */
export const storeIndicators = async (ticker: string, type: string, indicators: MovingAverages | RVolResult | VRSResult): Promise<void> => {
  try {
    const db = await getDatabase();
 ...
 */
export const getCachedIndicators = async (ticker: string, type: string, maxAge = 60 * 60 * 1000): Promise<MovingAverages | RVolResult | VRSResult | null> => {
  try {
    const db = await getDatabase();
    const transaction = db.transaction([STORES.INDICATORS], 'readonly');
    const store = transaction.objectStore(STORES.INDICATORS);

    // Try to use the compound index first, fallback to manual filtering
    let request;
    try {
      const index = store.index('ticker_type');
      request = index.get([ticker, type]);
    } catch (indexError) {
      console.warn('[DataLake] ticker_type index not found, using manual filtering:', indexError);
      // Fallback: get all by ticker and filter manually
      const tickerIndex = store.index('ticker');
      request = tickerIndex.getAll(ticker);
    }

    return new Promise((resolve, reject) => {
      request.onsuccess = () => {
        const results = Array.isArray(request.result) ? request.result : [request.result];
        const result = results.find(r => r && r.type === type) || (Array.isArray(request.result) ? null : request.result);
 ...
 * @returns {Promise<void>}
 */
export const storeStrategyResult = async (ticker: string, strategy: string, result: StrategyScore): Promise<void> => {
  try {
    const db = await getDatabase();
 ...
 * @returns {Promise<Object|null>}
 */
export const getLatestStrategyResult = async (ticker: string, strategy: string, maxAge = 5 * 60 * 1000): Promise<StrategyScore | null> => {
  try {
    const db = await getDatabase();
    const transaction = db.transaction([STORES.STRATEGY_RESULTS], 'readonly');
    const store = transaction.objectStore(STORES.STRATEGY_RESULTS);
    const index = store.index('ticker_strategy');
 ...
 */
export const migrateFromRVolDatabase = async (tickers: string[]): Promise<number> => {
  try {
 ...
 */
export const recordQueryPerformance = async (queryType: string, duration: number, resultCount: number): Promise<void> => {
  try {
    const db = await getDatabase();
 ...
 * @returns {Promise<Array>}
 */
const getTicksInTimeRange = async (ticker: string, startTime: number, endTime: number): Promise<Tick[]> => {
  try {
    const db = await getDatabase();
    const transaction = db.transaction([STORES.TICKS], 'readonly');
    const store = transaction.objectStore(STORES.TICKS);
 ...
 */
export const getDataLakeStats = async (): Promise<any> => {
  try {
    const db = await getDatabase();
    const stats = {
      version: DB_VERSION,
 ...
 */
export const cleanupOldData = async (symbols: string[] | null = null): Promise<any> => {
  try {
 ...
 */
const cleanupStore = async (db: IDBDatabase, operation: any): Promise<number> => {
 ...
 */
export const getTickerData = async (ticker: string): Promise<any> => {
  try {
    const now = Date.now();
 ...
 * @returns {Promise<void>}
 */
export const clearDataLake = async (): Promise<void> => {
  try {
 ...
 * @returns {Promise<void>}
 */
export const resetDataLake = async (): Promise<void> => {
  try {
 ...
 * @returns {Promise<any>} - Object mapping tickers to their data
 */
export const getMultipleTickerData = async (tickers: string[]): Promise<Record<string, any>> => {
  try {
    const results: Record<string, any> = {};

    // Process tickers in batches to avoid overwhelming the database
    const batchSize = 5;
    for (let i = 0; i < tickers.length; i += batchSize) {
      ...
      batchResults.forEach((result) => {
        if (result.status === 'fulfilled' && result.value) {
          results[result.value.ticker] = result.value.data;
        } else if (result.status === 'rejected') {
          console.warn(`[DataLake] Failed to get data for ticker:`, result.reason);
        }
      });
    }
    ...
    console.error('[DataLake] Error getting multiple ticker data:', error);
    return {};
  }
};
