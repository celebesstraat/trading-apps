...
import { Bar, HistoricalCandlesByDate } from '../types/types';
 ...
 */
export const storeCandles = async (ticker: string, date: string, candles: Bar[]): Promise<void> => {
  try {
 ...
 */
export const getCandles = async (ticker: string, date: string): Promise<Bar[] | null> => {
  try {
    const db = await initDatabase();
 ...
 */
export const getAllCandlesForTicker = async (ticker: string): Promise<HistoricalCandlesByDate[]> => {
  try {
    const db = await initDatabase();
    const transaction = db.transaction([STORE_NAME], 'readonly');
    const objectStore = transaction.objectStore(STORE_NAME);
 ...
 * @returns {Promise<Array>} Array of { date, candles } objects
 */
export const getRecentCandles = async (ticker: string, days = MAX_DAYS): Promise<HistoricalCandlesByDate[]> => {
  try {
 ...
 * @returns {Promise<void>}
 */
export const deleteCandles = async (ticker: string, date: string): Promise<void> => {
  try {
    const db = await initDatabase();
    const transaction = db.transaction([STORE_NAME], 'readwrite');
    const objectStore = transaction.objectStore(STORE_NAME);
 ...
 */
export const cleanupOldCandles = async (ticker: string): Promise<number> => {
  try {
    const allCandles = await getAllCandlesForTicker(ticker);
 ...
 */
export const cleanupAllTickers = async (tickers: string[]): Promise<number> => {
  let totalDeleted = 0;
 ...
 */
export const hasSufficientData = async (ticker: string, minDays = 10): Promise<boolean> => {
  try {
 ...
 */
export const getFirst5mCandles = async (ticker: string, days = MAX_DAYS): Promise<Bar[]> => {
  try {
 ...
 */
export const getTodayCandles = async (ticker: string, today: string): Promise<Bar[] | null> => {
  try {
